...
export const pureAddUser = (name: string, setError: Dispatch<SetStateAction<string | null>>, setName: Dispatch<SetStateAction<string>>, addUserCallback: (name: string) => void) => {
    if (name.trim() ==='') {setError('Ошибка! Введите имя!')} else {addUserCallback(name);
        setName('')}
    // если имя пустое - показать ошибку, иначе - добавить юзера и очистить инпут
}

export const pureOnBlur = (name: string, setError: Dispatch<SetStateAction<string | null>>) => { // если имя пустое - показать ошибку
